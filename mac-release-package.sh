...
FILES="${FILES} CoopNetwork.exe CoopNetworkservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe CoopNetwork.exe
cp installer/lib/JavaExe.exe CoopNetworkservice.exe
cp -a ${FILES} nxt
...
/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile BUD-client -name BUD-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.nxt.client.installer -Bmac.CFBundleName=BUD-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting NXT (YU63QW5EFW)" > installer/javapackager.log 2>&1
